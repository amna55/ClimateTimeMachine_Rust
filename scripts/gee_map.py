...
# Parse CLI arguments
# -----------------------
parser = argparse.ArgumentParser(description="Generate yearly average LST, anomaly tiles, and stats via GEE")
...
# -----------------------
print("Computing global dashboard stats...")

world = ee.Geometry.Polygon([[[-180, -90], [-180, 90], [180, 90], [180, -90]]])

mean_lst = fused_target_lst.reduceRegion(
...
mean_anomaly = absolute_anomaly.reduceRegion(
    reducer=ee.Reducer.mean(),
    geometry=world,
    scale=10000,
    maxPixels=1e13
...
max_anomaly = absolute_anomaly.reduceRegion(
...
min_anomaly = absolute_anomaly.reduceRegion(
...
hotspot_area = absolute_anomaly.gt(2).multiply(ee.Image.pixelArea()).reduceRegion(
...
coldspot_area = absolute_anomaly.lt(-2).multiply(ee.Image.pixelArea()).reduceRegion(
    reducer=ee.Reducer.sum(),
    geometry=world,
    scale=50000,
    maxPixels=1e13
...
land_area = ee.Image.pixelArea().reduceRegion(
    reducer=ee.Reducer.sum(),
    geometry=world,
    scale=50000,
    maxPixels=1e13
...
stats = ee.Dictionary({
    ...
    "min_anomaly_C": min_anomaly,
    "hotspot_percent": ee.Number(hotspot_area).divide(land_area).multiply(100),
    "coldspot_percent": ee.Number(coldspot_area).divide(land_area).multiply(100),
})

stats_info = stats.getInfo()
print("Dashboard Stats:")
print(json.dumps(stats_info, indent=2))
